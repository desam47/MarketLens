...
def test_live_quote_cache_suppresses_exact_duplicate_events():
    ...
    timestamp = datetime(2026, 9, 21, 15, 0, tzinfo=UTC)
...
def test_live_quote_cache_derives_spread_and_widening():
    cache = LiveQuoteCache()
    timestamp = datetime(2026, 9, 21, 15, 0, tzinfo=UTC)
    first = cache.update("AAPL", price=200.0, bid=199.99, ask=200.01, timestamp=timestamp, event_type="bbo")
    widened = cache.update("AAPL", price=200.0, bid=199.90, ask=200.10, timestamp=timestamp, event_type="bbo")

    assert first is not None
    assert first["spread_bps"] > 0
    assert widened is not None
    assert widened["spread_bps"] > first["spread_bps"]
    assert widened["spread_change_bps"] > 0
