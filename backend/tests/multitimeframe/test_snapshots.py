...
class TestTimeframeTrendSnapshotFields(unittest.TestCase):
    ...
    def test_direction_uses_trend_classification(self):
        ...
        snap = TimeframeTrendSnapshot(
            symbol="AAPL", timeframe=Timeframe.ONE_DAY, timestamp=datetime.now(),
            direction=TrendClassification.WEAK_BEARISH, score=-20.0,
            strength=0.25, confidence=0.5, data_quality="ok",
            strategy_version="v1.0",
        )
    ...
    def test_score_in_range(self):
        ...
        snap = TimeframeTrendSnapshot(
            symbol="AAPL", timeframe=Timeframe.ONE_DAY, timestamp=datetime.now(),
            direction=TrendClassification.BULLISH, score=55.0,
            strength=0.5, confidence=0.8, data_quality="ok",
            strategy_version="v1.0",
        )
...
class TestMultiTimeframeSnapshotFields(unittest.TestCase):
    ...
    def test_horizon_directions_use_trend_classification(self):
        ...
        from backend.multitimeframe.multi_timeframe_engine import ConfluenceDirection
        snap = MultiTimeframeSnapshot(
            symbol="AAPL", timestamp=datetime.now(), preset="swing",
            direction=ConfluenceDirection.DOWNTREND, strength=0.6,
            alignment_score=0.5, bullish_alignment=0.0, bearish_alignment=0.5,
            conflicting=2,
...
class TestSnapshotBehavior(unittest.TestCase):
    ...
    def _feed(self, engine: MultiTimeframeEngine, n: int = 120):
        base = datetime.now()
        for i in range(n):
            engine.update(100.0 + i * 0.5, 1_000_000,
    ...
    def test_snapshot_preset_name_matches_engine(self):
        ...
        for preset in ("day_trading", "swing", "all"):
            ...
            snap = engine.build_snapshot()
            self.assertIsNotNone(snap)
            self.assertEqual(snap.preset, preset,
                             f"Snapshot preset should be '{preset}'")
    ...
    def test_snapshot_history_accumulates(self):
        ...
        for i in range(150):
            engine.update(100.0 + i, 1_000_000,
                          base + timedelta(minutes=i))
