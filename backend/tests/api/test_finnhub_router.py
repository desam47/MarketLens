...
class TestFinnhubRouterResponses(unittest.TestCase):
    ...
    def test_health_down(self):
        with self._patch_service("get_company_profile", side_effect=RuntimeError("no connection")):
            r = self.client.get("/api/finnhub/health")
    ...
    def test_company_profile_200(self):
        profile = CompanyProfile(
            ...
            market_capitalization=2500000.0,
        )
    ...
    def test_company_profile_404(self):
        with self._patch_service("get_company_profile", side_effect=ValueError("no profile")):
            r = self.client.get("/api/finnhub/company/INVALID")
        self.assertEqual(r.status_code, 404)

    def test_company_profile_503(self):
        with self._patch_service("get_company_profile", side_effect=RuntimeError("rate limited")):
            r = self.client.get("/api/finnhub/company/AAPL")
        self.assertEqual(r.status_code, 503)
    ...
    def test_metrics_200(self):
        metrics = CompanyMetrics(
            symbol="AAPL", pe_basic_eps=28.5, beta=1.2, high_52w=200.0, low_52w=120.0
        )
    ...
    def test_metrics_404(self):
        with self._patch_service("get_company_metrics", side_effect=ValueError("no metrics")):
    ...
    def test_financials_200(self):
        fin = CompanyFinancials(
            symbol="AAPL", total_revenue=100000.0, net_income=25000.0, total_assets=200000.0
        )
    ...
    def test_insider_200(self):
        insider = [
            InsiderSentiment(symbol="AAPL", year=2024, month=1, change=50000, sentiment=0.3),
        ]
    ...
    def test_peers_404(self):
        with self._patch_service("get_peers", side_effect=ValueError("no peers")):
...
class TestFinnhubRouterCacheHeaders(unittest.TestCase):
    ...
    def test_recommendations_cache_header(self):
        self._assert_cache_header(
            "/api/finnhub/recommendations/AAPL",
