...
class TestAuxDataManagerDispatch(unittest.TestCase):
    ...
    def test_first_provider_succeeds(self):
        ...
        with patch.object(
            ...
            with patch(
                "backend.aux_data.services.manager._PROVIDER_CLASSES", {"fake": lambda: good}
            ):
                mgr = AuxDataManager()
                resp = mgr.get_news("AAPL")
    ...
    def test_falls_back_when_primary_fails(self):
        ...
        with patch.object(
            ...
            with patch(
                "backend.aux_data.services.manager._PROVIDER_CLASSES",
                {
                    "primary": lambda: bad,
    ...
    def test_unknown_provider_skipped(self):
        ...
        with patch.object(
            ...
            MagicMock(
                ...
                fallback_providers=["real"],
                rate_limit_per_minute=0,
            ),
        ):
            with patch(
                "backend.aux_data.services.manager._PROVIDER_CLASSES", {"real": lambda: good}
            ):
                mgr = AuxDataManager()
                resp = mgr.get_news("AAPL")
