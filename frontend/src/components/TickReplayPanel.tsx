...
export function TickReplayPanel() {
  ...
  const load = async () => { setLoading(true); try { const r = await api.getTickReplay(symbol.trim().toUpperCase()); setEvents(r.events); setCursor(0); } finally { setLoading(false); } };
  ...
  return <div className="card historical-replay-card">
    ...
    <div className="replay-controls"><label><span>Symbol</span><input value={symbol} maxLength={5} onChange={e => setSymbol(e.target.value.toUpperCase())} /></label><button className="btn btn-secondary" onClick={() => void load()} disabled={loading}>{loading ? 'Loading…' : 'Load ticks'}</button></div>
