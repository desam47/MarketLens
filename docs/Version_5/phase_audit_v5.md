...
| 5.4 | Analysis, comparisons, scenarios, and explanations | 🟡 IN PROGRESS | The typed `why_did_it_move`, `what_changed`, `compare_symbols`, `scenario_analysis`, and `historical_similarity` tools now provide evidence, baseline comparisons, bounded rankings, deterministic what-if outputs, and look-ahead-safe historical samples with explicit unknowns. Sensitivity remains. |
...
The first `why_did_it_move`, `what_changed`, `compare_symbols`, `scenario_analysis`, and `historical_similarity` slices are implemented and tested. Audit must include golden numerical fixtures, aligned-timeframe
