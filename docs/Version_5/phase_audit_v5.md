...
| 5.4 | Analysis, comparisons, scenarios, and explanations | 🟡 IN PROGRESS | The typed `why_did_it_move`, `what_changed`, and `compare_symbols` tools now provide evidence, baseline comparisons, and bounded rankings with explicit unknowns. Scenarios, similarity, and sensitivity remain. |
...
The first `why_did_it_move`, `what_changed`, and `compare_symbols` slices are implemented and tested. Audit must include golden numerical fixtures, aligned-timeframe
