...
| 5.4 | Analysis, comparisons, scenarios, and explanations | 🟡 IN PROGRESS | The typed `why_did_it_move`, `what_changed`, `compare_symbols`, and `scenario_analysis` tools now provide evidence, baseline comparisons, bounded rankings, and deterministic what-if outputs with explicit unknowns. Similarity and sensitivity remain. |
...
The first `why_did_it_move`, `what_changed`, `compare_symbols`, and `scenario_analysis` slices are implemented and tested. Audit must include golden numerical fixtures, aligned-timeframe
