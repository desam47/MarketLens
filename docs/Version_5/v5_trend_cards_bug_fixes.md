...
**Status:** ⚠️ PARTIAL — chart handoff shipped; change history, indicator attribution, and key-level context remain.
...
**Tests (chart handoff):** `TrendCard` click and keyboard activation call the handler, and the card is non-interactive without one; a `Dashboard` integration test asserts a card hands off `(symbol, timeframe)`. Change history, attribution, and key-level tests remain to be written with their sub-parts.
