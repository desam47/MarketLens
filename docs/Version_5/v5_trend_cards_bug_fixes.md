...
**Created:** 2026-09-24
**Last updated:** 2026-09-24 (all findings complete: TC-01 through TC-10, TC-06 folded)
**Status:** Re-prioritized from a trading-desk perspective: what would actually cost a trader money first, not just severity as originally filed. All findings are now implemented and covered by focused tests: TC-01's shared Trend and Confluence evidence contract, TC-02's end-to-end provider provenance, TC-03's measured short-horizon momentum display, TC-04's meaningful `Very Strong` boundary, TC-05's profile-aware score contract, TC-07's visible score/classification line, TC-08's explicit Confluence scope, and TC-09's four sub-parts (chart handoff, closed-bar change history, indicator attribution, and key-level context). TC-10's completeness check is satisfied by the per-finding card tests. TC-01 and TC-06 describe one underlying fix — a single server-owned evidence contract — and are tracked together under TC-01.
**Scorecard:** 9 ✅ COMPLETE, 0 ⚠️ PARTIAL, 0 ❌ NOT STARTED, 0 🟡 DEFERRED, 1 folded (TC-06 → TC-01).
...
| TC-09 | Medium | Workflow | Cards lack change history, indicator attribution, key-level context, and chart handoff | Code-read | ✅ COMPLETE |
| TC-10 | Medium | Tests | Direct TrendCard regressions are incomplete | Code-read | ✅ COMPLETE |
...
**Implementation (2026-09-24):** `EngineRegistry.dispatch_bar` now carries `provider`, and all three `MarketDataIngestionService` dispatch sites forward `getattr(bar, "provider", None)`. The kwarg reaches every `bar:{tf}` consumer safely — `TrendEngine.update` accepts it, while the realtime WS dispatcher and the regime `bar_update` absorb it through `**kwargs`, so no callback breaks. `TrendEngine._merged_provider` keeps a single agreeing source on a live aggregate and reports `mixed` the moment sources disagree (a missing provider on either side counts as disagreement); `_aggregate_live_bar` applies it when folding each closed 1m bar into a higher timeframe. `TrendCard`'s `formatProvider` renders `mixed`, the `aggregated_from_*` / `live_from_1m` derived sources, and any raw provider in a compact readable form, hiding `unknown` rather than showing it.

**Tests:** `dispatch_bar` forwards `provider` to a registered callback and defaults it to `None` when omitted (`backend/tests/test_engine_seeding.py`); `_merged_provider` and live-aggregate provenance cover agree / disagree / missing-side / both-missing / whitespace (`backend/tests/trend/test_trend_engine.py::TestTrendProviderProvenance`); the card renders mixed, derived-aggregate, and live-1m labels, falls back to the trend-level provider when evidence carries none, and hides a bare `unknown` (`frontend/src/components/TrendCard.test.tsx`).
...
**Status:** ✅ COMPLETE — all four sub-parts shipped: chart handoff, closed-bar change history, indicator attribution, and key-level context.
...
**Scope note (2026-09-24 investigation):** this finding is four independent sub-parts of unequal cost. **Chart handoff** was cheap and had no backend gap — `SymbolPage` already accepts `initialTimeframe` and `App.navigateTo('symbol', {symbol, timeframe})` exists — so it shipped first. The other three each need real backend work: **change history** because `trend_history` is appended on every `update()` (polling frequency), not per closed bar, so a naive count would report ticks, not bars — the exact trap this finding flags; **indicator attribution** because `_calculate_trend` computes per-component `(signal, weight)` pairs but discards them into the composite score; and **key-level context** because indicator level accessors (SuperTrend flip price, Bollinger edges) are not exposed on the signal or payload.

**Implementation — chart handoff (2026-09-24):** `TrendCard` accepts an optional `onOpenChart` and, when present, becomes a keyboard-operable `role="button"` (Enter/Space) with an `Open <symbol> <timeframe> chart` label; `Dashboard` threads `onOpenChart(symbol, trend.timeframe)` and `App` opens the Symbol page at that timeframe. Cards render exactly as before when no handler is supplied, so no existing usage regresses.

**Implementation — change history (2026-09-24):** `TrendEngine` now records a direction RUN — one entry per direction change, keyed by the `_bar_counts[timeframe]` closed-bar index — from `_update_from_bar`, which fires exactly once per closed bar (it is dup-guarded and is never on the tick path). This is the fix for the polling trap: `trend_history` grows on every tick-frequency `update()`, so counting its entries would report ticks; counting closed-bar runs reports bars. `get_trend_change_history` returns `direction`, `bars_in_state` (consecutive closed bars, inclusive of the run's first), `previous_direction`, `changed_at`, and `witnessed_change` — the last is `True` only when this engine actually observed the transition, so a run older than the tracked log is honestly shown as a lower bound rather than a fabricated start. The Trend API serializes this as `change_history` (with `changed_at` in the dashboard timezone), and `TrendCard` renders `Held 7 bars`, `New (was uptrend)`, or `Held 5+ bars` accordingly.

**Implementation — indicator attribution + key-level context (2026-09-24):** `_calculate_trend` now tags each scoring component `(name, signal, weight)` and, after combining, emits an `attribution` list where each entry's `contribution` is that component's share of the -100..+100 score (`signal * weight / total_weight * 100`); the entries sum to the raw score and are sorted by absolute impact. `_analyze_timeframe_trend` also collects `key_levels` from the located SuperTrend instance (flip price = active band, direction, band-distance in ATRs) and Bollinger instance (upper/middle/lower). Both ride on `TrendSignal` (optional, default empty) and serialize on the Trend payload. Short timeframes that omit SuperTrend/Bollinger simply return no such levels. `TrendCard` renders a compact key-level line and an expandable **Why this score** panel that lists each component's signed contribution; the panel stops click/keydown propagation so expanding it never triggers the chart handoff.

**Tests:** chart handoff — `TrendCard` click and keyboard activation call the handler, the card is non-interactive without one, and a `Dashboard` integration test asserts a card hands off `(symbol, timeframe)`. Change history — engine unit tests for consecutive-bar counting, an observed transition (previous direction + `changed_at`), a fresh run reporting one bar, and an end-to-end feed proving it counts closed bars and observes a reversal; API tests for the serialized payload and the pre-bar `None`; and card tests for the observed, fresh-reversal, lower-bound, and absent states. Attribution + key levels — engine tests that a full-stack timeframe's contributions sum to the score and are sorted by impact, that SuperTrend/Bollinger levels are exposed, and that a short timeframe exposes neither; an API pass-through test; and card tests for the signed component list, the level line, the empty state, and that expanding the panel does not navigate.

### TC-10 — Direct TrendCard regressions are incomplete

**Status:** ✅ COMPLETE
**Where:** Dashboard tests mock `TrendCard` (`frontend/src/pages/Dashboard.test.tsx:10`); direct coverage now exists in `frontend/src/components/TrendCard.test.tsx` across every truth-state contract.

Focused engine tests cover closed-bar processing and confidence bounds, and registry tests cover warm-up / dispatch behaviour. Direct card tests now cover evidence state/age, unavailable and warming states, short-horizon momentum wording, provider provenance (TC-02), score/classification display with signs and threshold tooltip (TC-07), chart handoff click/keyboard/timeframe (TC-09), closed-bar change history (TC-09), and indicator attribution + key-level context (TC-09). A `Dashboard` integration test keeps the selected Confluence preset and card scope aligned (TC-08) and asserts the chart handoff. Every truth-state contract from TC-01 through TC-09 now has a direct card test, so this completeness check is satisfied.
...
| 2026-09-24 | TC-01 | ⚠️ PARTIAL | folded into `03e5b3a` | `backend/api/trend/router.py`, `backend/tests/api/test_trend_api.py`, `frontend/src/components/TrendCard.tsx`, `frontend/src/components/TrendCard.test.tsx`, `frontend/src/services/api.ts`, `frontend/src/styles/App.css` | 119 focused backend tests passed; 5 TrendCard tests passed; production frontend build passed | Added the server-owned Trend evidence contract and rendered it directly. API as-of time now comes from each timeframe's own metadata; daily midnight source stamps normalize to the regular close while retaining their raw timestamp. Confluence adoption remains. |
...
| 2026-09-24 | TC-07 | ✅ COMPLETE | `101823e` | `frontend/src/services/api.ts`, `frontend/src/components/TrendCard.tsx`, `frontend/src/styles/App.css`, `frontend/src/components/TrendCard.test.tsx`, `docs/Version_5/v5_trend_cards_bug_fixes.md` | `classify_score` boundaries already covered by `backend/tests/trend/test_classification.py`; 20 TrendCard tests passed (6 new score/classification cases); tsc --noEmit clean | Surfaced the server's existing `score`/`classification` as a compact signed `Score +82 · Strong bullish` line with a threshold tooltip. The card displays the server-derived bucket verbatim; agreement stays a separate field. |
| 2026-09-24 | TC-09 (chart handoff) | ⚠️ PARTIAL | `969e475` | `frontend/src/App.tsx`, `frontend/src/pages/Dashboard.tsx`, `frontend/src/pages/Dashboard.test.tsx`, `frontend/src/components/TrendCard.tsx`, `frontend/src/components/TrendCard.test.tsx`, `frontend/src/styles/App.css`, `docs/Version_5/v5_trend_cards_bug_fixes.md` | 23 TrendCard tests passed (3 new handoff cases); 5 Dashboard tests passed (1 new handoff integration); tsc --noEmit clean | Shipped the cheapest, backend-free TC-09 sub-part: clicking (or Enter/Space on) a card opens the Symbol chart at that card's timeframe. Change history, indicator attribution, and key-level context remain; each needs backend work (see the scope note in the TC-09 entry). |
| 2026-09-24 | TC-09 (change history) | ⚠️ PARTIAL | `117a2cb` | `backend/trend/trend_engine.py`, `backend/api/trend/router.py`, `backend/tests/trend/test_trend_engine.py`, `backend/tests/api/test_trend_api.py`, `frontend/src/services/api.ts`, `frontend/src/components/TrendCard.tsx`, `frontend/src/components/TrendCard.test.tsx`, `frontend/src/styles/App.css`, `docs/Version_5/v5_trend_cards_bug_fixes.md` | 83 focused backend tests passed (9 new: change-history unit + end-to-end + API contract); 27 TrendCard tests passed (4 new change-history cases); tsc --noEmit clean | Recorded direction runs per closed bar (`_bar_counts` index) from `_update_from_bar`, not per tick — the polling-trap fix. Serialized `change_history` and rendered `Held N bars` / `New (was …)` / `Held N+ bars`. Indicator attribution + key-level context still remain. |
| 2026-09-24 | TC-09 (attribution + key levels), TC-10 | ✅ COMPLETE | `9b509c9` | `backend/trend/trend_engine.py`, `backend/api/trend/router.py`, `backend/tests/trend/test_trend_engine.py`, `backend/tests/api/test_trend_api.py`, `frontend/src/services/api.ts`, `frontend/src/components/TrendCard.tsx`, `frontend/src/components/TrendCard.test.tsx`, `frontend/src/styles/App.css`, `docs/Version_5/v5_trend_cards_bug_fixes.md` | 108 focused backend tests passed (6 new: attribution/key-level engine + API pass-through); 31 TrendCard tests passed (4 new attribution/key-level cases); tsc --noEmit clean | Named each scoring component and emitted a score-share `attribution` list plus SuperTrend/Bollinger `key_levels`; rendered a signed **Why this score** panel and a key-level line, with propagation stopped so the panel never triggers chart handoff. Made the confluence reuse of `_build_trend_payload` defensive for engines without the new method. Completes TC-09 and satisfies TC-10's completeness check. |
